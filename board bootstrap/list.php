      ...
      <!-- 페이지네이션 -->
        <nav aria-label="페이지 내비게이션">
          <ul class="pagination justify-content-center">
            <?php if ($s_pageNum > 1): ?>
              <li class="page-item">
                <a class="page-link" href="?page=<?php echo $s_pageNum - 1; ?>&search=<?php echo $search; ?>">&laquo; 이전</a>
              </li>
            <?php endif; ?>
            <?php for ($p = $s_pageNum; $p <= $e_pageNum; $p++): ?>
              <li class="page-item">
                <a class="page-link" href="?page=<?php echo $p; ?>&search=<?php echo $search; ?>" class="<?php echo $p == $page ? 'active' : ''; ?>"><?php echo $p; ?></a>
              </li>
            <?php endfor; ?>
            <?php if ($e_pageNum < $total_page): ?>
              <li class="page-item">
                <a class="page-link" href="?page=<?php echo $e_pageNum + 1; ?>&search=<?php echo $search; ?>">다음 &raquo;</a>
              </li>
            <?php endif; ?>
          </ul>
        </nav>
      ...
      <form id="searchForm" method="GET" action="./list.php">
        <div id="btn">
          <div class="input-group text-center">
            <input type="search" id="search" name="search" placeholder="검색(제목+내용)" class="form-control">
            <input type="submit" value="검색하기" id="search_btn" class="btn btn-secondary">
